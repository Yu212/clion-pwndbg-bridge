...
internal class AddressInspectionTimelineStore(private val project: Project) {
    ...
    fun fetchFullAt(
        ...
        service.executeCommandsSequential(
            ...
            val telescopeByLine = splitByLine(telescope.segments)
    ...
    private fun renderTelescope(lines: List<List<AnsiSegment>>, requestedLines: Int): List<AnsiSegment> {
        ...
        val shown = flattenLines(lines.take(shownCount))
    ...
    private fun splitByLine(segments: List<AnsiSegment>): List<List<AnsiSegment>> {
    ...
    private fun flattenLines(lines: List<List<AnsiSegment>>): List<AnsiSegment> {
        if (lines.isEmpty()) return emptyList()
